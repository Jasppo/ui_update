...
trendsTop_ui_sidebar <- function(id) {
  ...
  tagList(
    ...
    # myMeasure
    selectInput(NS(id, "myMeasure"),
                label=list(strong("Measure:"), 
                           actionButton("measureHelp", 
                                        label=inputStyles$helpIcon,
                                        style=inputStyles$myInputHelpButtonSty)),
                choices = choices$deathMeasures_noSMR, selected = "aRate"),
...
trendsTop_server <- function(id, r_global_inputs, urlParams = NULL) {
  moduleServer(id, function(input, output, session) {
    ...
    plot_step <- reactive(topCauses_trends(
      ...
      bindCache(r_global_inputs$myLHJ, input$myMeasure, input$myLogTrans, input$myN_topTrends, input$myLevShort, 

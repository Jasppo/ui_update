...
choices <- function() {
  ...
  # LHJs
  counties <- sort(c(constants$STATE, links$county$countyName))
  ...
  deathMeasures_ageTrend <- deathMeasures_noSMR[!deathMeasures_noSMR %in% c("aRate", "YLL.adj.rate")]
...
appText <- function() {
  ...
  # News and Updates
  updatesText <- read_docx("myData/appText/newsUseCCB_Word.docx") # Read in appText Word Doc
  updatesText <- docx_extract_tbl(updatesText, 1) # Extract table
  
  # Tab Information text
  tabInfo <- read_docx("myData/appText/tabInformation.docx") # Read in appText Word Doc

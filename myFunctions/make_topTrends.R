topCauses_trends <- function(
  ...
  # PREPARE DATASET ------------------------------------------------------------------------------------
  plot_df <- lazy_load(file.path("myData", constants$dataType, "datCounty.RDS")) %>%
    filter(county == myLHJ, Level == myLev, sex == "Total") %>%
    left_join(select(links$deathCause, causeCode, causeName, causeNameShort, topLevCode, topLevName), by = "causeCode") %>%
    filter(topLevCode != "Z") %>%
  ...
  y_title <- names(choices$deathMeasuresAll[choices$deathMeasuresAll == myMeasure])
  ...
  if (myLev == "lev1") {
    ...
    if (nrow(plot_df)==0) stop(appText$stopMessage)
    ...
    tPlot <- ggplot(filter(plot_df, year %in% yearRange), aes(x = year, y = measure, color = topLevName)) +
      scale_color_manual(values = plot_colors$topLev, drop = T, limits = force) +
    ...
    myLHJ_title <- ifelse(myLHJ == constants$STATE, myLHJ, paste0(myLHJ, " County"))

rankGeo <- function(myLHJ="CALIFORNIA", 
  ...
  if (myLHJ == constants$STATE) {
    ...
    tDat <- lazy_load(file.path("myData", constants$dataType, "datCounty.RDS")) %>% 
    ...
    sMeasure <- lazy_load(file.path("myData", constants$dataType, "datCounty_5year.RDS")) %>% 
      filter(county == myLHJ, yearG5 == current_years$currentYearG5, sex == mySex, causeCode == myCause) %>% 
      pull( {{ myMeasure }})
    
    # Load and process MSSA-Level data
    tDat <- lazy_load(file.path("myData", constants$dataType, "datComm.RDS")) %>% 
      mutate(plotter = !!as.symbol(myMeasure)) %>% 
  ...
  tDat_i <- tDat %>% 
    ...
    mutate(color = ifelse(lab == constants$STATE, "firebrick", "#0096FF"))

rankCause <- function(myCounty = "Los Angeles", 
  ...
  deathMeasures <- deathMeasures[deathMeasures %in% c("Ndeaths", "YLLper", "aRate", "mean.age", "SMR")]
  
  measuresLink <- data.frame(
    measure = unname(choices$deathMeasuresAll), 
    measureName = names(choices$deathMeasuresAll)
  )

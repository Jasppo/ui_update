trendGeneric <- function(myDemoGroup = "Sex",
  ...
  if (myDemoGroup == "Sex") {
    if (myYearGrouping == 1) {
      inDat <- lazy_load(file.path("myData", constants$dataType, "datCounty.RDS"))
    } else if (myYearGrouping == 3) {
      inDat <- lazy_load(file.path("myData", constants$dataType, "datCounty_3year.RDS"))
    } else if (myYearGrouping == 5) {
      inDat <- lazy_load(file.path("myData", constants$dataType, "datCounty_5year.RDS"))
    }
  } else if (myDemoGroup == "Age Group") {
    if (myYearGrouping == 1) {
      inDat <- lazy_load(file.path("myData", constants$dataType, "datState_AGE.RDS")) %>% 
        filter(sex=="Total", !is.na(ageGroup))
    } else if (myYearGrouping == 3) {
      inDat <- lazy_load(file.path("myData", constants$dataType, "datCounty_AGE_3year.RDS")) %>% 
        filter(sex=="Total", !is.na(ageGroup))
    }
  } else if (myDemoGroup == "Race/Ethnicity") {
    if (myYearGrouping == 1) {
      inDat <- lazy_load(file.path("myData", constants$dataType, "datState_RE.RDS")) %>% 
        filter(sex == "Total", !raceCode %in% c("Other", "Unknown", "Total")) %>% 
        left_join(select(links$race, raceCode, raceNameShort), by = "raceCode")
    } else if (myYearGrouping == 3) {
      inDat <- lazy_load(file.path("myData", constants$dataType, "datCounty_RE.RDS")) %>% 
        filter(sex == "Total", !raceCode %in% c("Other", "Unknown", "Total")) %>% 
        left_join(select(links$race, raceCode, raceNameShort), by = "raceCode")
    }
  } else if (myDemoGroup == "Education") {
    eduLink <- links$edu
    inDat <- lazy_load(file.path("myData", constants$dataType, "datCounty_EDU.RDS")) %>% 
      filter(sex == mySex) %>% 
